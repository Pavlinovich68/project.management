...
const ItrCalendarRow = ({row, index, year, month, callback}:{row:ICalendarRow, index: number, year: number, month: number, callback: CalendarCellEventCallbackExt}) => {
   ...
   return (
      <React.Fragment>
         <div key="row" className={classNames("flex justify-content-center", styles.calendarRow)}>
            ...
            {               
               row?.cells?.map((day) => <ItrCalendarCell key={`calendar-cell-id-${day.day}`} year={year} month={month} row={index} cell={day} rateId={row.rate_id} callback={event}/>)
            }
