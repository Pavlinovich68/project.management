...
const ItrCalendarRow = ({row, index, writeMode, dayType, recalcFooter, dict}:
   {row:ICalendarRow, index: number, writeMode: boolean, dayType: number | undefined, recalcFooter: any, dict: ICellDictionary}) => {
   ...
   return (
      <React.Fragment>
         <div key="row" className={classNames("flex justify-content-center", styles.calendarRow)}>
            ...
            {
               row?.cells?.map((day) => <ItrCalendarCell key={`calendar-cell-id-${day.id}`} row={index} cell={day} writeMode={writeMode} dayType={dayType} recalc={recalcRow} dict={dict}/>)
            }
